		...
		private static void DrawSlots(SpriteBatch spriteBatch)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				for (int j = 0; j < 4; j++)
				{
					...
					if (Utils.FloatIntersect((float)Main.mouseX, (float)Main.mouseY, 0f, 0f, (float)num, (float)num2, (float)Main.inventoryBackTexture.Width * Main.inventoryScale, (float)Main.inventoryBackTexture.Height * Main.inventoryScale))
					{
						player.mouseInterface = true;
		...
		public static void LootAll()
		{
			...
			for (int k = 0; k < 40; k++)
			{
				if (player.bank.item[k].type > 0)
				{
					...
					player.bank.item[k] = player.GetItem(Main.myPlayer, player.bank.item[k], false, false);
				}
			}
		}
					...
					if (player.inventory[i].maxStack > 1)
					{
						for (int j = 0; j < 40; j++)
						{
							...
							else if (player.bank.item[j].stack < player.bank.item[j].maxStack && player.inventory[i].IsTheSameAs(player.bank.item[j]))
							{
								...
								{
									num3 = player.bank.item[j].maxStack - player.bank.item[j].stack;
								}
								player.inventory[i].stack -= num3;
								player.bank.item[j].stack += num3;
								Main.PlaySound(7, -1, -1, 1);
								if (player.inventory[i].stack <= 0)
								{
									player.inventory[i].SetDefaults(0, false);
					...
					if (player.inventory[i].stack > 0)
					{
						...
						else
						{
							for (int m = 0; m < 40; m++)
							{
								...
								{
									Main.PlaySound(7, -1, -1, 1);
									player.bank.item[m] = player.inventory[i].Clone();
									player.inventory[i].SetDefaults(0, false);
									break;
								}
							}
