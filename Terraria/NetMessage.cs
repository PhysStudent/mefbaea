			...
			lock (NetMessage.buffer[num])
			{
				...
				if (remoteClient == -1)
				{
					if (msgType == 34 || msgType == 69)
					{
						...
						{
							if (num20 != ignoreClient && NetMessage.buffer[num20].broadcast && Netplay.Clients[num20].IsConnected())
							{
								try
								{
					...
					else if (msgType == 20)
					{
						...
						{
							if (num21 != ignoreClient && NetMessage.buffer[num21].broadcast && Netplay.Clients[num21].IsConnected() && Netplay.Clients[num21].SectionRange(number, (int)number2, (int)number3))
							{
								try
								{
				...
				if (Main.verboseNetplay)
				{
					...
					{
						byte arg_2CC1_0 = NetMessage.buffer[num].writeBuffer[num28];
					}
				}
		...
		public static void syncPlayers()
		{
			...
			for (int num2 = 0; num2 < 200; num2++)
			{
				if (Main.npc[num2].active && Main.npc[num2].townNPC && NPC.TypeToNum(Main.npc[num2].type) != -1)
				{
					...
					NetMessage.SendData(60, -1, -1, "", num2, (float)Main.npc[num2].homeTileX, (float)Main.npc[num2].homeTileY, (float)num3, 0, 0, 0);
				}
			}
			if (flag2)
			{
