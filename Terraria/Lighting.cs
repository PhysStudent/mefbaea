		...
		public static void PreRenderPhase()
		{
			...
			for (int n = num4; n < num5; n++)
			{
				...
				for (int num17 = num6; num17 < num7; num17++)
				{
					...
					{
						if ((!tile.active() || !Main.tileNoSunLight[(int)tile.type] || ((tile.slope() != 0 || tile.halfBrick()) && Main.tile[n, num17 - 1].liquid == 0 && Main.tile[n, num17 + 1].liquid == 0 && Main.tile[n - 1, num17].liquid == 0 && Main.tile[n + 1, num17].liquid == 0)) && lightingState3.r2 < Lighting.skyColor && (Main.wallLight[(int)tile.wall] || tile.wall == 73) && tile.liquid < 200 && (!tile.halfBrick() || Main.tile[n, num17 - 1].liquid < 200))
						{
							num18 = num;
							num19 = num2;
							num20 = num3;
					...
					{
						if ((!tile.active() || !Main.tileNoSunLight[(int)tile.type] || ((tile.slope() != 0 || tile.halfBrick()) && Main.tile[n, num17 - 1].liquid == 0 && Main.tile[n, num17 + 1].liquid == 0 && Main.tile[n - 1, num17].liquid == 0 && Main.tile[n + 1, num17].liquid == 0)) && lightingState3.r2 < num24 && (Main.wallLight[(int)tile.wall] || tile.wall == 73) && tile.liquid < 200 && (!tile.halfBrick() || Main.tile[n, num17 - 1].liquid < 200))
						{
							num18 = num24;
							num19 = num24 * 0.6f;
							num20 = num24 * 0.2f;
						...
						if (Main.tileLighted[(int)tile.type])
						{
									...
									else
									{
										...
										default:
											...
											case 125:
											{
												...
												num19 = (lightingState3.g2 = 0.3f * num40);
												num20 = (lightingState3.b2 = 0.6f * num40);
												goto IL_304B;
											}
											case 126:
												if (tile.frameX < 36)
							...
							if (type <= 286)
							{
								...
								else if (type <= 238)
								{
									...
									else
									{
										if (tile.frameY < 36)
										{
											float num43 = (float)Main.rand.Next(28, 42) * 0.005f;
											num43 += (float)(270 - (int)Main.mouseTextColor) / 700f;
							...
							else if (type <= 350)
							{
								if (type <= 318)
								{
									...
									case 318:
									{
										int num44 = n - (int)(tile.frameX / 18);
										int num45 = num17 - (int)(tile.frameY / 18);
										int num46 = num44 / 2 * (num45 / 3);
										num46 %= Main.cageFrames;
										bool flag = Main.jellyfishCageMode[(int)(tile.type - 316), num46] == 2;
										if (tile.type == 316)
										{
											if (flag)
											{
								...
								else
								{
									...
									{
										float num47 = 0.5f;
										num47 += (float)(270 - (int)Main.mouseTextColor) / 1500f;
										num47 += (float)Main.rand.Next(0, 50) * 0.0005f;
										num18 = 1f * num47;
										num19 = 0.5f * num47;
										num20 = 0.1f * num47;
										goto IL_304B;
									...
									case 350:
									{
										...
										float num49 = (float)(-(float)Math.Cos(((int)(num48 / 6.283) % 3 == 1) ? num48 : 0.0) * 0.1 + 0.1);
										num18 = num49;
										num19 = num49;
										num20 = num49;
										goto IL_304B;
								...
								default:
									...
									else
									{
										if (tile.frameX < 54)
										{
											...
											num50 += (float)(270 - (int)Main.mouseTextColor) / 700f;
											switch (tile.frameX / 54)
											{
											case 1:
												num18 = 0.7f;
							...
							if (tile.type != 349 || tile.frameX >= 36)
							{
								...
								num52 += (float)(270 - (int)Main.mouseTextColor) / 1000f;
								num18 = 0.1f;
								num19 = 0.2f + num52 / 2f;
								num20 = 0.7f + num52;
							}
					...
					{
						float num53 = (num18 + num19 + num20) / 3f;
					...
					if (tile.lava() && tile.liquid > 0)
					{
						if (Lighting.RGB)
						{
							float num54 = (float)(tile.liquid / 255) * 0.41f + 0.14f;
							num54 = 0.55f;
							num54 += (float)(270 - (int)Main.mouseTextColor) / 900f;
							if (lightingState3.r2 < num54)
							{
								lightingState3.r2 = num54;
							}
							if (lightingState3.g2 < num54)
							{
								lightingState3.g2 = num54 * 0.6f;
							}
							if (lightingState3.b2 < num54)
							{
								lightingState3.b2 = num54 * 0.2f;
							}
						}
						else
						{
							float num55 = (float)(tile.liquid / 255) * 0.38f + 0.08f;
							num55 += (float)(270 - (int)Main.mouseTextColor) / 2000f;
					...
					{
						int num56 = n - Lighting.firstToLightX;
						int num57 = num17 - Lighting.firstToLightY;
			...
			{
				int num58 = (int)current.Key.X - Lighting.firstTileX + Lighting.offScreenTiles;
				int num59 = (int)current.Key.Y - Lighting.firstTileY + Lighting.offScreenTiles;
				if (num58 >= 0 && num58 < Main.screenWidth / 16 + Lighting.offScreenTiles * 2 + 10 && num59 >= 0 && num59 < Main.screenHeight / 16 + Lighting.offScreenTiles * 2 + 10)
				{
					Lighting.LightingState lightingState4 = Lighting.states[num58][num59];
					...
					if (num58 < Lighting.minX)
					{
						Lighting.minX = num58;
					}
					if (num58 > Lighting.maxX)
					{
						Lighting.maxX = num58;
					}
					if (num59 < Lighting.minY)
					{
						Lighting.minY = num59;
					}
					if (num59 > Lighting.maxY)
					{
						Lighting.maxY = num59;
					}
				}
			...
			if (Main.player[Main.myPlayer].accOreFinder)
			{
				...
				for (int num60 = 0; num60 < 419; num60++)
				{
					...
					{
						Main.player[Main.myPlayer].bestOre = num60;
					}
				}
			}
