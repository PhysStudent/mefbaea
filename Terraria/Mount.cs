	...
	public class Mount
	{
		...
		public void UpdateFrame(Player mountedPlayer, int state, Vector2 velocity)
		{
			...
			case 8:
				if (state == 0 || state == 1)
				{
					...
					float arg_185_0 = position.Y / 16f;
					float num2 = 0f;
			...
			case 0:
				...
				{
					float num11 = (float)this._data.idleFrameDelay;
					if (this._type == 5)
					{
						num11 *= 2f - 1f * this._fatigue / this._fatigueMax;
					}
					int num12 = (int)((float)(this._idleTime - this._idleTimeNext) / num11);
					if (num12 >= this._data.idleFrameCount)
					{
					...
					{
						this._frame = this._data.idleFrameStart + num12;
					}
					if (this._type == 5)
					{
			...
			case 1:
			{
				...
				if (type != 6)
				{
					if (type != 9)
					{
						if (type != 13)
						{
							num13 = Math.Abs(velocity.X);
						}
						else
						{
							num13 = (this._flipDraw ? velocity.X : (-velocity.X));
						}
					}
			...
			case 2:
				...
				{
					float num14 = this._fatigue / this._fatigueMax;
					this._frameExtraCounter += 6f - 4f * num14;
			...
			case 11:
			{
				...
				if (mountedPlayer.whoAmI == Main.myPlayer && mountedPlayer.velocity.X != 0f)
				{
					...
					for (int j = 0; j < 200; j++)
					{
						...
						{
							Vector2 v = nPC2.position + nPC2.Size * Utils.RandomVector2(Main.rand, 0f, 1f) - minecartMechPoint;
							num10 += v.ToRotation();
							num9++;
							int num11 = Projectile.NewProjectile(minecartMechPoint.X, minecartMechPoint.Y, v.X, v.Y, 591, 0, 0f, mountedPlayer.whoAmI, (float)mountedPlayer.whoAmI, 0f);
		...
		public void Draw(List<DrawData> playerDrawData, int drawType, Player drawPlayer, Vector2 Position, Color drawColor, SpriteEffects playerEffect, float shadow)
		{
			...
			if (!flag)
			{
				...
				if (texture2D2 != null)
				{
					item = new DrawData(texture2D2, Position, new Rectangle?(value), color, num5, origin, scale2, effect, 0);
					item.shader = Mount.currentShader;
				}
				playerDrawData.Add(item);
			}
