...
namespace Terraria.GameContent.Biomes
{
	internal class CaveHouseBiome : MicroBiome
	{
		...
		public override bool Place(Point origin, StructureMap structures)
		{
			...
			{
				int y;
				bool flag4 = this.FindSideExit(new Rectangle(current4.X + current4.Width, current4.Y + 1, 1, current4.Height - 2), false, out y);
				if (flag4)
				{
					list4.Add(new Point(current4.X + current4.Width - 1, y));
				}
				flag4 = this.FindSideExit(new Rectangle(current4.X, current4.Y + 1, 1, current4.Height - 2), true, out y);
				if (flag4)
				{
					list4.Add(new Point(current4.X, y));
				}
			}
			...
			{
				int num11 = current10.Width / 8;
				int num12 = current10.Width / (num11 + 1);
				int num13 = GenBase._random.Next(2);
				for (int n = 0; n < num11; n++)
				{
					int num14 = (n + 1) * num12 + current10.X;
					switch (n + num13 % 2)
					{
					case 0:
					{
						int num15 = current10.Y + Math.Min(current10.Height / 2, current10.Height - 5);
					...
					case 1:
					{
						...
						for (int num16 = -1; num16 < 2; num16++)
						{
							...
							{
								Tile expr_E29 = GenBase._tiles[num16 + num14, num17 + num15];
								expr_E29.frameX += 54;
							}
						}
						break;
				...
				int num18 = current10.Width / 8 + 3;
				WorldGen.SetupStatueList();
				while (num18 > 0)
				{
					int num19 = GenBase._random.Next(current10.Width - 3) + 1 + current10.X;
					int num20 = current10.Y + current10.Height - 2;
			...
			{
				int num22 = current12.Height - 1 + current12.Y;
				...
				int num24 = current12.X + 2;
				while (num24 <= current12.X + current12.Width - 2 && !(flag6 = WorldGen.AddBuriedChest(num24, num22, 0, false, style2)))
				{
					num24++;
				}
				if (flag6)
			...
			if (!flag6)
			{
				foreach (Rectangle current13 in list)
				{
					...
					int num27 = current13.X + 2;
					while (num27 <= current13.X + current13.Width - 2 && !(flag6 = WorldGen.AddBuriedChest(num27, num25, 0, false, style3)))
					{
						num27++;
					}
					if (flag6)
			...
			if (item == CaveHouseBiome.BuildData.Jungle && this._sharpenerCount < GenBase._random.Next(2, 5))
			{
				...
				{
					int num30 = current14.Height - 2 + current14.Y;
					for (int num31 = 0; num31 < 10; num31++)
					{
						int num32 = GenBase._random.Next(2, current14.Width - 2) + current14.X;
					...
					int num33 = current14.X + 2;
					while (num33 <= current14.X + current14.Width - 2 && !(flag7 = WorldGen.PlaceTile(num33, num30, 377, true, true, -1, 0)))
					{
						num33++;
					}
					if (flag7)
			...
			if (item == CaveHouseBiome.BuildData.Desert && this._extractinatorCount < GenBase._random.Next(2, 5))
			{
				...
				{
					int num34 = current15.Height - 2 + current15.Y;
					for (int num35 = 0; num35 < 10; num35++)
					{
						int num36 = GenBase._random.Next(2, current15.Width - 2) + current15.X;
					...
					int num37 = current15.X + 2;
					while (num37 <= current15.X + current15.Width - 2 && !(flag8 = WorldGen.PlaceTile(num37, num34, 219, true, true, -1, 0)))
					{
						num37++;
					}
					if (flag8)
