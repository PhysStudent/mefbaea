...
namespace Terraria.GameContent.Liquid
{
	internal class LiquidRenderer
	{
		...
		private static int[] WATERFALL_LENGTH = new int[]
		{
			10,
			3,
			2
			...
			fixed (LiquidRenderer.LiquidCache* ptr = &this._cache[1])
			{
				...
				for (int i = rectangle.X; i < rectangle.X + rectangle.Width; i++)
				{
					for (int j = rectangle.Y; j < rectangle.Y + rectangle.Height; j++)
					{
						...
						if (tile == null)
						{
							tile = LiquidRenderer.EMPTY_TILE;
						}
				...
				for (int num24 = rectangle.X; num24 < rectangle.X + rectangle.Width; num24++)
				{
					for (int num25 = rectangle.Y; num25 < rectangle.Y + rectangle.Height; num25++)
					{
						if (ptr2->VisibleType == 1 && ptr2->HasVisibleLiquid && Dust.lavaBubbles < 200)
						{
							...
							{
								Dust.NewDust(new Vector2((float)(num24 * 16), (float)(num25 * 16)), 16, 16, 35, 0f, 0f, 0, Color.White, 1f);
							}
							if (this._random.Next(350) == 0)
							{
								int num26 = Dust.NewDust(new Vector2((float)(num24 * 16), (float)(num25 * 16)), 16, 8, 35, 0f, 0f, 50, Color.White, 1.5f);
								Main.dust[num26].velocity *= 0.8f;
								Dust expr_1069_cp_0 = Main.dust[num26];
								expr_1069_cp_0.velocity.X = expr_1069_cp_0.velocity.X * 2f;
								Dust expr_1087_cp_0 = Main.dust[num26];
								expr_1087_cp_0.velocity.Y = expr_1087_cp_0.velocity.Y - (float)this._random.Next(1, 7) * 0.1f;
								if (this._random.Next(10) == 0)
								{
									Dust expr_10C3_cp_0 = Main.dust[num26];
									expr_10C3_cp_0.velocity.Y = expr_10C3_cp_0.velocity.Y * (float)this._random.Next(2, 5);
								}
				...
				for (int i = drawArea.X; i < drawArea.X + drawArea.Width; i++)
				{
					for (int j = drawArea.Y; j < drawArea.Y + drawArea.Height; j++)
					{
						if (ptr2->IsVisible)
						{
							...
							float scale = (*ptr2).Opacity * Math.Min(1f, isBackgroundDraw ? 1f : (globalAlpha * LiquidRenderer.DEFAULT_OPACITY[(int)(*ptr2).Type]));
							colors.BottomLeftColor *= scale;
							colors.BottomRightColor *= scale;
							colors.TopLeftColor *= scale;
							colors.TopRightColor *= scale;
							Main.tileBatch.Draw(this._liquidTextures[num], new Vector2((float)(i << 4), (float)(j << 4)) + drawOffset + liquidOffset, new Rectangle?(sourceRectangle), colors, Vector2.Zero, 1f, SpriteEffects.None);
						}
						ptr2++;
					}
				}
