...
namespace Terraria.GameContent.UI.Elements
{
	internal class UIAchievementListItem : UIPanel
	{
		...
		protected override void DrawSelf(SpriteBatch spriteBatch)
		{
			...
			Vector2 stringSize = ChatManager.GetStringSize(Main.fontItemStack, this._achievement.Description, baseScale2, num2);
			if (stringSize.Y > 38f)
			{
			...
			vector.X += 4f;
			vector.X += 17f;
			ChatManager.DrawColorCodedStringWithShadow(spriteBatch, Main.fontItemStack, this._achievement.FriendlyName, vector, color, 0f, Vector2.Zero, baseScale, num2, 2f);
			vector.X -= 17f;
			Vector2 position = value + Vector2.UnitY * 27f;
			this.DrawPanelBottom(spriteBatch, position, num2, color3);
			position.X += 8f;
			position.Y += 4f;
			ChatManager.DrawColorCodedStringWithShadow(spriteBatch, Main.fontItemStack, this._achievement.Description, position, color2, 0f, Vector2.Zero, baseScale2, num2 - 10f, 2f);
			if (flag)
			{
