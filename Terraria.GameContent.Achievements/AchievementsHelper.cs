...
namespace Terraria.GameContent.Achievements
{
	public class AchievementsHelper
	{
		...
		private static void OnPlayerEnteredWorld(Player player)
		{
			...
			for (int num2 = 0; num2 < player.armor.Length; num2++)
			{
				if (player.armor[num2].wingSlot > 0)
				{
			...
			bool flag = true;
			for (int num3 = 0; num3 < player.extraAccessorySlots + 3 + 5; num3++)
			{
				if (player.dye[num3].type < 1 || player.dye[num3].stack < 1)
				{
					flag = false;
				}
			}
